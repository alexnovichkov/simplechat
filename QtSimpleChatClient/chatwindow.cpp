...
void ChatWindow::attemptConnection()
{
    ...
    QSettings se;
    auto hostAddresses = se.value(QStringLiteral("server_addresses")).toStringList();
    if (hostAddresses.isEmpty())
        hostAddresses << QStringLiteral("127.0.0.1");

    auto hostAddress = QInputDialog::getItem(this, tr("Chose Server")
        , tr("Server Address"), hostAddresses
    );
    ...
    if (!hostAddresses.contains(hostAddress)) {
        ...
        se.setValue(QStringLiteral("server_addresses"), hostAddresses);
    }
    ...
    m_chatClient->connectToServer(QHostAddress(hostAddress), SERVER_PORT);
}
...
void ChatWindow::connectedToServer()
{
    ...
    QSettings se;
    QString newUsername = se.value(QStringLiteral("username")).toString();
    if (newUsername.isEmpty())
        newUsername = QInputDialog::getText(this, QStringLiteral("Chose Username"), QStringLiteral("Username"));
    if (newUsername.isEmpty()) {
        ...
        m_chatClient->login(newUsername);
    }
}
...
void ChatWindow::loginFailed(const QString &reason)
{
    ...
    QMessageBox::critical(this, QStringLiteral("Error"), reason);
